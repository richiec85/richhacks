...
description: "No network after adding a NIC, corosync loses quorum, ZFS pool won't import on boot — the full Proxmox post-mortem and the permanent fix."
pubDate: 2026-07-01
pillar: "homelab"
postNumber: 1
status: "resolved"
...
> **Update (July 2026):** Proxmox VE 9 shipped a built-in tool, `pve-network-interface-pinning`, that automates the permanent fix in Step 3 below — one command generates the same MAC-pinned `.link` file and updates `/etc/network/interfaces` for you:
>
> ```bash
> pve-network-interface-pinning generate --interface enp1s0 --target-name cluster0
> ```
>
> If you're on PVE9+, that's the fast path. I've left the manual method below as-is, because it's exactly what that tool is doing under the hood, and it's still the right approach if you're on an older version or just want to understand the mechanism rather than trust a script.
