...
{/*
  DRAFT NOTE — delete this comment before publishing.
  Four machine-specific values to confirm against your own boxes first:
   1. The OLD vs NEW interface names (I've used enp2s0 -> enp5s0 as the example).
   2. The real MAC address in the .link file (placeholder aa:bb:cc:... below).
   3. The exact kernel version you pinned — the post shows `kernel list` first so the
      reader picks the real one, so nothing here is wrong, but drop in your string if you
      want it concrete.
   4. Pool names (I've used `rpool` for root and `tank` for data) — swap for your actual names.
  Everything else — the commands, the causal chain, the fixes — is accurate as written.
*/}
